...
enum PermissionOnboardingApp {
    static func launch(forceShow: Bool = false) {
        let force = forceShow || ProcessInfo.processInfo.environment["OPENARA_FORCE_ONBOARDING"] == "1"
        guard force || !PermissionDiagnostics.current().allGranted else {
            return
        }
...
final class PermissionContentController: NSViewController {
    ...
    private func configureUI() {
        ...
        completionLabel.font = AppFonts.sans(size: 15, weight: .medium)
...
final class GuidancePlaceholderView: NSView {
    init() {
        ...
        label.font = AppFonts.sans(size: 12, weight: .medium)
...
final class PermissionAccessoryPanelView: NSView {
    ...
    private func setup() {
        ...
        instructionLabel.font = AppFonts.sans(size: 14, weight: .medium)
...
final class DraggableAppTileView: NSView, NSDraggingSource {
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        let attributes: [NSAttributedString.Key: Any] = [
            .font: AppFonts.sans(size: 15, weight: .medium),
...
enum Branding {
    static func makeAppIconImage(size: CGFloat) -> NSImage {
        if let logo = AppLogo.load() {
            let scaled = NSImage(size: NSSize(width: size, height: size))
            scaled.lockFocus()
            logo.draw(
                in: CGRect(origin: .zero, size: scaled.size),
                from: .zero,
...
final class StatusChipView: NSView {
    init(text: String, foreground: NSColor, background: NSColor) {
        ...
        label.font = AppFonts.sans(size: 14, weight: .medium)
