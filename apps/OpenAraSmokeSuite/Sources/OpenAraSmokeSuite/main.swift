...
@main
enum OpenAraSmokeSuite {
    static func main() throws {
        let productsDirectory = try locateProductsDirectory()
        let fixtureURL = productsDirectory.appendingPathComponent("OpenAraFixture")
        let serverURL = productsDirectory.appendingPathComponent("OpenAra")
        let appName = "OpenAraFixture"
    ...
    private static func runCursorIdleSmoke(serverURL: URL, appName: String) throws {
        ...
        environment["OPENARA_VISUAL_CURSOR"] = "1"
        environment["OPENARA_VISUAL_CURSOR_OBSERVATION_FILE"] = observationURL.path
    ...
    private static func cursorObservationFileURL() -> URL {
        URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("openara-smoke", isDirectory: true)
