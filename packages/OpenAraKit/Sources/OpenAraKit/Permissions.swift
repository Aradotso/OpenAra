...
public struct PermissionDiagnostics: Sendable {
    ...
    public let rawAccessibilityFromTCC: Bool?
    public let rawScreenCaptureFromTCC: Bool?
    public let rawAccessibilityFromSystemAPI: Bool?
    public let rawScreenCaptureFromSystemAPI: Bool?

    public init(
        accessibilityTrusted: Bool,
        screenCaptureGranted: Bool,
        rawAccessibilityFromTCC: Bool? = nil,
        rawScreenCaptureFromTCC: Bool? = nil,
        rawAccessibilityFromSystemAPI: Bool? = nil,
        rawScreenCaptureFromSystemAPI: Bool? = nil
    ) {
        self.accessibilityTrusted = accessibilityTrusted
        self.screenCaptureGranted = screenCaptureGranted
        self.rawAccessibilityFromTCC = rawAccessibilityFromTCC
        self.rawScreenCaptureFromTCC = rawScreenCaptureFromTCC
        self.rawAccessibilityFromSystemAPI = rawAccessibilityFromSystemAPI
        self.rawScreenCaptureFromSystemAPI = rawScreenCaptureFromSystemAPI
    }

    public static func current() -> PermissionDiagnostics {
        let persisted = TCCAuthorizationStore.current
        let axRaw = AXIsProcessTrusted()
        let screenRaw = CGPreflightScreenCaptureAccess()

        if ProcessInfo.processInfo.environment["OPENARA_FORCE_ONBOARDING"] == "1" {
            return PermissionDiagnostics(
                accessibilityTrusted: false,
                screenCaptureGranted: false,
                rawAccessibilityFromTCC: persisted.accessibility,
                rawScreenCaptureFromTCC: persisted.screenRecording,
                rawAccessibilityFromSystemAPI: axRaw,
                rawScreenCaptureFromSystemAPI: screenRaw
            )
        }

        return PermissionDiagnostics(
            accessibilityTrusted: persisted.accessibility ?? axRaw,
            screenCaptureGranted: persisted.screenRecording ?? screenRaw,
            rawAccessibilityFromTCC: persisted.accessibility,
            rawScreenCaptureFromTCC: persisted.screenRecording,
            rawAccessibilityFromSystemAPI: axRaw,
            rawScreenCaptureFromSystemAPI: screenRaw
...
public enum PermissionSupport {
    public static let bundleDisplayName = "OpenAra"
    ...
    public static func isOpenAraBundleIdentifier(_ bundleIdentifier: String?) -> Bool {
    ...
    private static func preferredInstalledAppBundleURL() -> URL? {
        ...
        for prefix in homebrewPrefixes() {
            appendCandidate(prefix
                ...
                .appendingPathComponent("openara", isDirectory: true)
                .appendingPathComponent(releaseAppBundleName, isDirectory: true)
            )
