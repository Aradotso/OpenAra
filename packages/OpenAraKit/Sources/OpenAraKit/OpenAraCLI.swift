...
public let openAraDefaultInterCallDelay: TimeInterval = 1

public struct OpenAraCLIError: LocalizedError, Equatable {
    ...
    public var errorDescription: String? {
        var lines = [message]
        lines.append("")
        lines.append(openAraHelpText(command: helpCommand))
        return lines.joined(separator: "\n")
    }
}

public func parseOpenAraCLI(arguments: [String]) throws -> OpenAraCLICommand {
    ...
    case "-h", "--help", "help":
        if arguments.count > 2 {
            throw OpenAraCLIError(message: "help accepts at most one command", helpCommand: nil)
        }

        return .help(command: arguments.dropFirst().first)
    case "-v", "--version", "version":
        guard arguments.count == 1 else {
            throw OpenAraCLIError(message: "version does not accept any arguments", helpCommand: nil)
        }
    ...
    default:
        ...
        throw OpenAraCLIError(message: "Unknown command: \(first)", helpCommand: nil)
    }
}

public func openAraHelpText(command: String? = nil) -> String {
    switch command {
    case nil:
        return """
        Open Computer Use
    ...
    case "call":
        return """
        Usage:
          openara call <tool> [--args '<json-object>']
          openara call <tool> [--args-file <path>]
          openara call --calls '<json-array>' [--sleep <seconds>]
          openara call --calls-file <path> [--sleep <seconds>]

        Examples:
          openara call list_apps
          openara call get_app_state --args '{"app":"TextEdit"}'
          openara call --calls '[{"tool":"get_app_state","args":{"app":"TextEdit"}},{"tool":"press_key","args":{"app":"TextEdit","key":"Return"}}]'
        ...
        Sequence runs sleep \(formatOpenAraDelay(openAraDefaultInterCallDelay)) between successful operations by default.
        """
    case "turn-ended":
        return """
        Usage:
          openara turn-ended [--previous-notify <argv>] [payload]
...
private func parseSimpleCommand(
    name: String,
    arguments: [String],
    result: OpenAraCLICommand
) throws -> OpenAraCLICommand {
    ...
    throw OpenAraCLIError(message: "\(name) does not accept any arguments", helpCommand: name)
}

private func parseTurnEnded(arguments: [String]) throws -> OpenAraCLICommand {
    if arguments.count == 1, let option = arguments.first, option == "-h" || option == "--help" {
    ...
    while index < arguments.count {
        ...
        case "--previous-notify":
            ...
            guard valueIndex < arguments.count else {
                throw OpenAraCLIError(message: "--previous-notify requires a value", helpCommand: "turn-ended")
            }
            index = valueIndex
        case "-h", "--help":
            throw OpenAraCLIError(message: "turn-ended help must be requested as `openara turn-ended --help`", helpCommand: "turn-ended")
        default:
            if argument.hasPrefix("-") {
                throw OpenAraCLIError(message: "Unknown turn-ended option: \(argument)", helpCommand: "turn-ended")
            }

            guard payload == nil else {
                throw OpenAraCLIError(message: "turn-ended accepts at most one payload argument", helpCommand: "turn-ended")
            }
...
private func parseSnapshot(arguments: [String]) throws -> OpenAraCLICommand {
    ...
    if arguments.isEmpty {
        throw OpenAraCLIError(message: "snapshot requires an app name or bundle identifier", helpCommand: "snapshot")
    }

    throw OpenAraCLIError(message: "snapshot accepts exactly one <app> argument", helpCommand: "snapshot")
}

private func parseCall(arguments: [String]) throws -> OpenAraCLICommand {
    if arguments.count == 1, let option = arguments.first, option == "-h" || option == "--help" {
    ...
    var interCallDelay = openAraDefaultInterCallDelay
    ...
    while index < arguments.count {
        ...
        case "-h", "--help":
            throw OpenAraCLIError(message: "call help must be requested as `openara call --help`", helpCommand: "call")
        default:
            if argument.hasPrefix("-") {
                throw OpenAraCLIError(message: "Unknown call option: \(argument)", helpCommand: "call")
            }

            guard toolName == nil else {
                throw OpenAraCLIError(message: "call accepts at most one tool name", helpCommand: "call")
            }
    ...
    if hasSequenceInput {
        if callsJSON != nil, callsFile != nil {
            throw OpenAraCLIError(message: "Use either --calls or --calls-file, not both", helpCommand: "call")
        }
    ...
    if argumentsJSON != nil, argumentsFile != nil {
        throw OpenAraCLIError(message: "Use either --args or --args-file, not both", helpCommand: "call")
    }

    if interCallDelay != openAraDefaultInterCallDelay {
        throw OpenAraCLIError(
    ...
    guard let toolName else {
        throw OpenAraCLIError(message: "call requires a tool name or --calls/--calls-file", helpCommand: "call")
    }
...
private func parseOptionValue(
    ...
    let valueIndex = index + 1
    guard valueIndex < arguments.count else {
        throw OpenAraCLIError(message: "\(option) requires a value", helpCommand: "call")
    }
